...
def get_latest_price(pair: TradingPair) -> Decimal:
    """
    Return the latest USD market price for the trading pair's base asset.

    This reads public market data only.
    It does not access wallet secrets, private keys, or signing material.
    """
    latest = (
        MarketPrice.objects
    ...
    if latest.price_usd <= 0:
        raise ValidationError("The current market price is invalid.")
...
def validate_order(
    *,
    ...
    """
    Validate the basic trading rules for an order.

    This function performs validation only.
    It does not execute, sign, or broadcast transactions.
    """
